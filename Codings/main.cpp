...
int SCAN_FREQUENCY = 25;
    ...
    for (int scanning_freq = 0; scanning_freq < SCAN_FREQUENCY; scanning_freq++){
		for (int i = 8, j = 1; i > 0 && j < 9; i--, j++){
            ...
            SPI_Write2(i,led_buf[j-1]);
		ThisThread::sleep_for(0.000001);   // 0.1 sec delay 
		}  
    }
}
...
void mag_to_buf()
{
    ...
    //  }
    pc.printf("magnitudes[%d] *1000 = %i \r\n", 0, int(magnitudes[0] * 1000));
    //pc.printf("magnitudes[%d]*100 ~= %i \r\n", 1, int(magnitudes[1] * 100));
    for (int i = 1; i < N; i++)
    {
        //pc.printf("magnitudes[%d]*1000 = %i  ", i, int(magnitudes[i] * 1000));
        if (magnitudes[i] > xmax)
        {
            xmax = magnitudes[i];
            pc.printf("magnitudes[%d] *1000 = %i \r\n", i, int(magnitudes[i] * 1000));
			pc.printf("xmax*1000 = %i \r\n", int(xmax*1000));
        }
    }
    ...
    if (magnitudes[0] - xmax > 1.0)
    {
        ...
        for (int i = 1; i < 9; i++)
        {
            ...
            pc.printf("temp_float*1000000 = %i  ",int(temp_float*1000000));
            led = (int)((temp_float * 100) / (magnitudes[0] * 70 / 100));
			pc.printf("led = %d \r\n",led);
            if (led > 8) { led = 8; }
